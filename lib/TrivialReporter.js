...
jasmine.TrivialReporter.prototype.reportSpecResults = function(spec) {
  ...
  for (var i = 0; i < resultItems.length; i++) {
    ...
    if (result.passed && !result.passed()) {
      this.reportFailure(result, specDiv);
    }
  }
...
jasmine.TrivialReporter.prototype.reportFailure = function(result, specDiv) {
  var messageHtml = result.matcherName == "toEqual" ? this.formatToEqualResult(result) : result.message;

  specDiv.appendChild(this.createDom('div', {className: 'resultMessage fail'}, messageHtml));
  specDiv.appendChild(this.createDom('div', {className: 'stackTrace'}, result.trace.stack));
};

jasmine.TrivialReporter.prototype.formatToEqualResult = function(result) {
  var message = this.createDom("div", {},
      "Expected",
      this.createDom("span", {className: "expected result"}, jasmine.Matchers.pp(result.expected)),
      "but got",
      this.createDom("span", {className: "actual result"}, jasmine.Matchers.pp(result.actual))
     );

  var mismatches = result.details.mismatchKeys;
  mismatches.push.apply(mismatches, result.details.mismatchValues);

  message.appendChild(this.createDom("div", {className: "mismatches"}, "Differences"));

  for(i=0;i<mismatches.length;i++) {
    message.appendChild(this.createDom("div", {className: 'mismatch'}, mismatches[i]));
  }
